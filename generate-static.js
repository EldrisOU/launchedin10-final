...
function injectSchema(html, pageType, pageData) {
    // Strip any pre-existing JSON-LD blocks from the shell to prevent duplication
    // (the SPA shell index.html has Organization+WebSite; SSG pages replace it with page-specific schema)
    ...
    if (pageType === 'Article') {
        ...
        // BreadcrumbList for blog posts: Home → Blog/Category → Post
        ...
        if (pageData.category) {
            breadcrumbItems.push({
                ...
                "item": `https://launchedin10.co.uk/blog/${pageData.categorySlug || ''}/`
            });
